...
namespace SportsApp.Adapter
{
    class MatchListAdapter : BaseAdapter<Match>
    {
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            if (row == null)
            {
                row = LayoutInflater.From(context).Inflate(Resource.Layout.list_row_match, null, false);
            }
            ...
            txt1.Text = matches[position].Team1 + " vs " + matches[position].Team2;
            txt2.Text = matches[position].Description;
            DateTime date = Common.DateFromMilliseconds(matches[position].MatchDate);
            txt3.Text = "Match Date: " + date.ToLongDateString();
            return row;
        }
    }
}